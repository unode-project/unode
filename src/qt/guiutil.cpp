...
{
    // Convert Unode:// to Unode:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Unode://"))
    {
        uri.replace(0, 12, "Unode:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Unode.lnk";
}
...
{
    return GetAutostartDir() / "Unode.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Unode-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Unode-qt [" + tr("command-line options") + "]                     " + "\n";
