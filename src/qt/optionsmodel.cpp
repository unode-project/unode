...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeUnodeAmount"))
        settings.setValue("nAnonymizeUnodeAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeUnodeAmount = settings.value("nAnonymizeUnodeAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeUnodeAmount"))
        SoftSetArg("-anonymizeUnodeamount", settings.value("nAnonymizeUnodeAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeUnodeAmount:
            nAnonymizeUnodeAmount = value.toInt();
            settings.setValue("nAnonymizeUnodeAmount", nAnonymizeUnodeAmount);
            emit anonymizeUnodeAmountChanged(nAnonymizeUnodeAmount);
            break;
        default:
            break;
        }
