...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Unode
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Unode
    // Mac: ~/Library/Application Support/Unode
    // Unix: ~/.Unode
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Unode";
#else
    fs::path pathRet;
...
void createConf()       //Automatic Unode.conf generation
{
    srand(time(NULL));
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Unode.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "unoded.pid"));
